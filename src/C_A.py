...
class CA:
    ...
    def __init__(self, name, robot_name):
        ...
        self.num_fatal_collisions = 0 # Number of not avoided collisions
        ...
        self.safety_radius = rospy.get_param('~safety_radius')  
        ...
        for parameter in parameters:
            ...
            elif parameter['name'] == "ORI_ERROR":
                self.ORI_ERROR = parameter['value']
        ...
        self.results_msg.safety_radius = self.safety_radius
        ...
        self.results_msg.ORI_ERROR = self.ORI_ERROR
        ...
        self.pub_safety_marker = rospy.Publisher('/' + self.robot_name + '/safety_sphere', Marker, queue_size=1)
    ...
    def send_goto_strategy(self, position_x, position_y, position_z, keep_position, linear_velocity):
        ...
        goto_thread_thread = threading.Thread(target=goto_thread)
        goto_thread_thread.start()    
    
    def position_check(self):
        # Increase iterator to navigate through the robot path after reaching the waypoint
        if self.obj_distance <= self.safety_radius:
    ...
    def update_robot_position(self,msg):
        ...
        with self.update_lock:
            ...
            # Create CA and safety spheres        
            self.create_sphere(self.pos_x, self.pos_y, self.pos_z, "CA sphere")
            self.create_sphere(self.pos_x, self.pos_y, self.pos_z, "safety sphere")
        ...
        with self.detection_lock:
            ...
            if self.enable_CA and self.initial_reached:
                ...
                if self.obs_distances[robot_number] <= self.CRIT_DIST:
                    ...
                    if self.obs_distances[robot_number] <= self.safety_radius:
    ...
    def potential_fields(self, obs_distance, obs_pos_x, obs_pos_y, obs_pos_z):
        ...
        # alpha of the final vector
        alpha = atan2(vfy, vfx)

        # alpha of the final vector from the robot's pose
        alpha = atan2(sin(alpha - self.yaw), cos(alpha - self.yaw))
        ...
        # If vf points behind the robot recalculate alpha angle to allow the robot to move backwards
        if len(self.critical_robots) != 0:
            if alpha <= -pi/2:
                alpha = pi + alpha
                vf_behind = True
        ...
        # if alpha is inside the threshold and vf points behind then move backwards while turning
        if abs(alpha) <= self.ORI_ERROR and vf_behind:
            self.set_vel_msg.twist.linear.x = -self.DES_VEL
            self.set_vel_msg.twist.angular.z = self.K_ROT_MIN*self.ROT_VEL*alpha
        
        # if alpha is inside the threshold and vf points forward then move forward while turning
        elif abs(alpha) <= self.ORI_ERROR and not vf_behind:
            self.set_vel_msg.twist.linear.x = self.DES_VEL
            self.set_vel_msg.twist.angular.z = self.K_ROT_MIN*self.ROT_VEL*alpha
        
        # Otherwise stop and rotate until alpha is inside the threshold           
        else:
            self.set_vel_msg.twist.linear.x = 0.0
            self.set_vel_msg.twist.angular.z = self.K_ROT_MAX*self.ROT_VEL*alpha
    ...
    def reciprocal_velocity(self, obs_pos_x, obs_pos_y, obs_pos_z, obs_vel_x, obs_vel_y, obs_vel_z, robot_number):
        ...
        if self.start_RVO_simulation:
            ...
            self.sim = rvo_wrapper.PyRVOSimulator(self.time_step, self.CRIT_DIST, self.num_robots,
                        self.time_horizon, self.safety_radius, self.DES_VEL, velocity)
            self.agent = self.sim.addAgent(position, self.CRIT_DIST, self.num_robots, self.time_horizon,
                        self.safety_radius, self.DES_VEL, velocity)
        ...
        if robot_number not in self.neighbors_id:
            ...
            self.neighbors[robot_number] = self.sim.addAgent(neighbor_position, self.CRIT_DIST, self.num_robots,
                                            self.time_horizon, self.safety_radius, self.DES_VEL, neighbor_velocity)
        ...
        # if alpha is inside the threshold and vf points behind then move backwards while turning
        if abs(alpha) <= self.ORI_ERROR and vx_behind:
            self.set_vel_msg.twist.linear.x = -velocity_x
            self.set_vel_msg.twist.angular.z = self.K_ROT_MIN*self.ROT_VEL*alpha
        
        # if alpha is inside the threshold and vf points forward then move forward while turning
        elif abs(alpha) <= self.ORI_ERROR and not vx_behind:
            self.set_vel_msg.twist.linear.x = velocity_x
            self.set_vel_msg.twist.angular.z = self.K_ROT_MIN*self.ROT_VEL*alpha
    ...
    def create_sphere(self, sphere_x, sphere_y, sphere_z, sphere):
        ...
        if sphere == "CA sphere" and self.enable_CA:
            marker.scale.x = self.CRIT_DIST
            marker.scale.y = self.CRIT_DIST
            marker.scale.z = self.CRIT_DIST

            if self.enable_CA and self.initial_reached:
                if any(item is not None and item <= self.safety_radius for item in self.obs_distances):
                    marker.color.r = 1.0
                    marker.color.g = 0.0
                    marker.color.b = 0.0
                    marker.color.a = 0.25
        ...
        # Create safety sphere with sphere radius
        elif sphere == "safety sphere" and self.enable_CA:
            marker.scale.x = self.safety_radius
            marker.scale.y = self.safety_radius
            marker.scale.z = self.safety_radius
            ...
            marker.lifetime = rospy.Duration()
            self.pub_safety_marker.publish(marker)
